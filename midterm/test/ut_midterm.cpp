...
TEST(ComplexShapes, ChildOperation)
{
    ...
    Shape * test=new ComplexShapes(&shapes);
    
    ASSERT_EQ(0,complexShapes->numberOfChild());
    ASSERT_EQ(0,test->numberOfChild());
    ...
    ASSERT_NEAR(3.14,test->getChild(0)->area(),0.01);
    ASSERT_NEAR(6.28,test->getChild(0)->perimeter(),0.01);
